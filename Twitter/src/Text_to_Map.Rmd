...
SearchTrend <- read_csv('./Data/SearchTrend.csv', col_names = TRUE, skip = 2) %>% 
  ...
  mutate(Time = as.Date(Month, "%y-%b"))

min <- as.Date("2004-01-01")
max <- as.Date("2021-05-01")

Trend <- ggplot(SearchTrend,aes(x=Time, y=Total_Searches,color = "darkgoldenrod1", group=1)) +
  geom_line() +
...
ppal2 <- colorFactor("RdYlBu", domain=SelSchools$Selectivity)
color_Selectivity = ppal2(SelSchools$Selectivity)
icon.pop <- pulseIcons(color = ifelse(SelSchools$Selectivity == "elite", "#E64141", 
                       ...
                       heartbeat = ifelse(SelSchools$Selectivity == "elite", "0.5", 
                                      ifelse(SelSchools$Selectivity =="highly selective", "0.7", "0.8")))
...
popup_sb2 <- paste0("University: ", as.character(SelSchools$INSTNM),
                  ";  City: ", as.character(SelSchools$CITY),
                  ";  Admission Rate: ",as.character(SelSchools$ADM_RATE))

# Map
map_title2 <- tags$p(tags$style('p {color: black; font-size: 15px}'),
                    tags$b('#CancelStudentDebt Tweets in the US'))
...
leaflet() %>%
  ...
  addLegend(position = "bottomright",
            ...
            values = states_shape_tweets$Avg.Sentiment, 
            opacity = 0.7) %>% 
  addLayersControl(baseGroups = c('Twitter Sentiments', 'Selective Institutions'),
