...
sc_repay %>%
  ...
  # plot
  ggplot(aes(x = cohort_year, y = median_rates, group = class, color = class)) +
  geom_point() + geom_line(aes(group = cohort_year)) +
  ...
  scale_color_viridis_d(name = "Income Range:",
                        labels = c("Overall", "High Income", "Middle Income", "Low Income")) +
  scale_y_continuous(labels = scales::dollar_format(suffix = "%", prefix = "")) +
  scale_y_continuous(labels = scales::percent_format()) +
  ## themes
...
sc_repay %>% #filter(cohort_year %in% 2006:2008) %>%
  ...
  geom_density_ridges(scale = 1.5, color = "gray") +
  
  # labels
  ggtitle("Proportion of Fiscal Cohort Whose Loan Balances Have Declined") +
  ylab("Fiscal Cohort") + xlab(NULL) +
  scale_fill_brewer(name = "Family Income Range:", labels = c("High", "Middle", "Low"), direction = -1) +
